...
char *C_HL_KEYWORDS[] = {
	...
	"struct", "union", "typedef", "static", "enum", "case", "noreturn", "extern", "inline",
 ...
 * separates a number, indicating the number is not part of a variable or type name
 * e.g. `int16_t`
 *
 *
 * @param c
...
void highlight_syntax(t_row *row) {
	...
	// `true` if the prev row has an unclosed ml comment i.e. `hl_open_comment` == 1
	...
	while (i < row->size) {
		...
		if (single_begin_len && !in_str && !in_comment /* don't hl comment inside of ml comment */) {
		...
		if (prev_delimited) {
			...
			for (j = 0; keywords[j]; j++) {
				...
				if (!strncmp(&row->render[i], keywords[j], kw_len) &&
					...
					memset(&row->highlight[i], is_kw2 ? HL_KEYWORD2 : HL_KEYWORD1, kw_len);
					i += kw_len;
...
int map_syntax_to_color(int hl) {
	switch (hl) {
		case HL_COMMENT: case HL_ML_COMMENT: return 36;
