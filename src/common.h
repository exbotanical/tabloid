...
#define CTRL_KEY(k)                                                          \
  ((k)&0x1f) /**< Mandate the ctrl binding that exits the program by setting \
                upper 3 bits to 0 */
...
typedef struct syntax_config {
  char* f_type; /**< file type that will be displayed to the user in the status
                   bar */
  char** f_match;  /**< an array of char arrays where ea str contains a pattern
                      to match a file name against */
  char** keywords; /**< an array of char arrays where ea str contains a keyword
                      match; kw2 is terminated with a pipe char */
  char* single_ln_comment_begin; /**< track how a single-line comment begins for
                                    the given file type*/
  char* multi_ln_comment_begin;  /**< track how a multi-line comment begins for
                                    the given file type*/
  char* multi_ln_comment_end; /**< track how a multi-line comment ends for the
                                 given file type*/
  int flags; /**< bit field indicating whether to hl numbers, string for given
                file type */
} syntax_config;

struct tty_conf {
  struct termios
      og_tty; /**< Pointer ref for storing original termios configurations */
  int screenrows;
  int screencols;
  int curs_x, curs_y; /**< Cursor indices - chars on Cartesian plane */
  int render_x;       /**< Index of render buffer */
  int rowoff;         /**< Row offset - tracks which row of the file the user is
                         scrolled to */
