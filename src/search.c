...
/**
 * @brief Eagerly searches content text by evaluating against each query
...
void eager_search(char* query, int key) {
  ...
  if (line_hl) {
    memcpy(T.row[lineno_hl].highlight, line_hl, T.row[lineno_hl].rsize);
  ...
  if (key == '\r' || key == ESCAPE) {
    ...
    // i.e. we set `dir` to forward and only backward if the user explicitly
    // chooses to
    dir = 1;
  }
  ...
  for (i = 0; i < T.num_rows; i++) {
    ...
    if (match) {
      ...
      memset(
          ...
          &row->highlight[match - row->render], HL_SEARCH_MATCH, strlen(query));
